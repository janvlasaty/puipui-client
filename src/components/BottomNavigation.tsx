...
export const BottomNavigation: React.FC<BottomNavigationProps> = ({ activeTab, onTabChange }) => {
  ...
  const tabs = [
    { id: 'chat', label: t('nav.direct'), icon: ChatCircleIcon },
    { id: 'rooms', label: t('nav.rooms'), icon: ChatsIcon },
    { id: 'map', label: t('nav.map'), icon: MapTrifoldIcon },
    { id: 'vibes', label: t('nav.vibes'), icon: StarIcon },
  ] as const

  return (
    <div className="fixed bottom-0 left-0 right-0 flex items-center justify-center pointer-events-none">
        <div className="mb-8 pointer-events-auto bg-background/70 backdrop-blur-sm rounded-full shadow-lg border border-border/50 px-1 py-1 flex gap-0">
          {tabs.map((tab) => {
            const Icon = tab.icon
            return (
              <button
